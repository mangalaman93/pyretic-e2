...
dest2 = E2NF("h7", 1,'dst3', inp_load_estimate=(l4), switch_placed='s9')

# create pipelets
pipe1 = E2Pipelet("pipelet-1")
pipe1.add_nodes_from([source1, NF1, NF2, dest1])
...
pipe3.add_nodes_from([source3, NF1, NF3, dest2])
pipe3.add_edges_from([
    (source3, NF1, {'filter': match(dstport = 70)}),
    (NF1, NF3, {'filter':match(dstport = 70)}),
    (NF3, dest2, {'filter': match(dstport = 70)})
    ])
...
src_s3 = LoadGenerator.src(h7.IP(), 70, tcp=False, num_request=100000)
h3.cmd(src_s1 + " &")
...
def main():
  pipelets = [pipe1, pipe2, pipe3, pipe4]
